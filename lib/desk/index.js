...
Desk.prototype.validate = function (message, settings) {
    ...
    || this.ensure(settings.apiKey, 'api_key');
};
...
Desk.prototype.identify = function (identify, settings, callback) {
  ...
  this._getUser(url, { email : email }, apiKey, function (err, user) {
    if (err) return callback(err);
    if (!user) self._createUser(identify, apiKey, callback);
...
Desk.prototype._getUser = function (url, query, apiKey, callback) {
  return this
...
Desk.prototype._updateUser = function (url, id, identify, apiKey, callback) {
  return this
...
Desk.prototype._createUser = function (url, identify, apiKey, callback) {
  return this
    .post(url + '/customers')
    .auth(apiKey, 'X')
    .query({ reload: true })
...
function formatTraits (identify) {
  ...
  var traits = { 
    external_id    : identify.proxy('traits.id'), //possibly just identify.id?
