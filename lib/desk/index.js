...
Desk.prototype.identify = function (identify, settings, callback) {
  ...
  var id = identify.userId || identify.uid(); //falls back to email
  var self  = this;

  //add some way of checking if id has fallen back to email
  //so that we can skip one of the gets

  this._getUser({external_id : id}, settings, function (err, user) {
    if (err) return callback(err);
    if (!user) {
      self._getUser({email : email}, settings, function (err, user) {
        if (err) return callback(err);
...
Desk.prototype._getUser = function (searchParams, settings, callback) {
...
Desk.prototype._updateUser = function (id, identify, settings, callback) {
  var url = this.siteUrl.replace(/<siteName>/, settings.siteName);
  return this
...
Desk.prototype._createUser = function (identify, settings, callback) {
  var url = this.siteUrl.replace(/<siteName>/, settings.siteName);
  return this
    .post(url + '/customers')
    .auth(settings.email, settings.password)
...
function formatTraits (identify) {
  ...
  var traits = { 
    external_id    : identify.proxy('userId'), 
